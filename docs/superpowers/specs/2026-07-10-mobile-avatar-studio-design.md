...
1. An uninitialized deployment accepts only a one-time, high-entropy setup URL generated by the deployment command and shown privately in the local terminal. The public site never exposes this URL or token.
2. On the first setup visit, the owner chooses the shared Workshop password. The Worker atomically stores the verifier and permanently closes the setup path so another visitor cannot claim it.
3. Access after initialization requires that shared password; there are no accounts, passkeys, recovery flow, or approval steps.
4. The repository, browser bundle, browser storage, telemetry, and logs never contain the password or its plaintext equivalent.
5. D1 stores only a unique salt and PBKDF2-SHA-256 password verifier. The bootstrap-token verifier and session-signing key are encrypted Worker secrets.
6. The browser sends setup and login passwords only over HTTPS. The Worker excludes request bodies from logs and discards plaintext immediately after deriving the verifier.
7. Successful login returns a random, signed, revocable session in an `HttpOnly`, `Secure`, `SameSite=Strict` cookie with a 12-hour maximum lifetime.
8. Failed setup and login attempts use rate limits and increasing retry delays. Repeated failures emit a sanitized security alarm without recording attempted passwords.
9. Password reset requires a newly deployed one-time setup token. Rotating the verifier, rotating the signing key, or activating the kill switch invalidates all Workshop sessions.
10. A server-side schedule and kill switch control Workshop availability.
11. Workshop commands use the same instruction field and constrained operation schema.
12. Allowed operations cover design tokens, spacing, typography, component order, visibility, sizing, copy, and supported layouts.
13. Arbitrary HTML, JavaScript, URLs, shell commands, workflow definitions, and repository instructions are rejected.
14. The Worker repeats validation; client validation is not trusted.
15. Valid changes publish immediately to a versioned Experimental configuration in Cloudflare KV or D1.
16. Authenticated Workshop devices see the preview configuration immediately.
17. `publish experimental` promotes the current Workshop preview to all Experimental users.
18. `undo`, `rollback`, and `restore version N` provide immediate recovery.
19. Solid mode never reads Workshop configuration.
